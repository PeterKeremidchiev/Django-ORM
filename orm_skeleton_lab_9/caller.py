...
def product_quantity_ordered():
    product_quantity = (Product.objects.annotate(total_quantity_ordered=Sum('orderproduct__quantity'))
                        .exclude(total_quantity_ordered=None)).order_by("-total_quantity_ordered")
    result = []
    for product in product_quantity:
        result.append(f"Quantity ordered of {product.name}: {product.total_quantity_ordered}")
...
def ordered_products_per_customer():
    ...
    # prefetched_orders = Order.objects.all().order_by("id")
...
def give_discount():
    discount = F("price") * 0.7
    query = Q(is_available=True) & Q(price__gt=3)
    Product.objects.filter(query).update(price=discount)
